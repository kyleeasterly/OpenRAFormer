...
- **Look at `production` → `queued` BEFORE ordering.** If the building you want is already in the queue or in progress, it is coming — do NOT order it again, and do NOT cancel it. Just wait for it.
- **NEVER cancel the item that is in progress.** It is already paid for and about to exist. `cancel_production` removes the most recently queued copies first — but if your count reaches the one under construction it WILL cancel that too, wasting all its build time. The order result tells you exactly what was cancelled: read it.
- Cancel ONLY in one situation: the same building appears 2 or more times in the waiting list. Then `cancel_production` with a small `count` trims the extras.
- The normal number of cancels in a whole game is close to zero. If you are cancelling every turn, you are the problem.
