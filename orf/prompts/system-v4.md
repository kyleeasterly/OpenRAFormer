...
- **NEVER cancel the item that is in progress.** It is already paid for and about to exist. The game protects it: `cancel_production` only removes WAITING copies behind it.
- Cancel ONLY in one situation: the same building appears 2 or more times in the waiting list. Then `cancel_production` trims the extras from the end of the queue.
