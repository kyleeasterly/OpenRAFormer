...
## The queue remembers — order once, then wait

- **Each building needs to be ordered exactly ONCE.** If it shows in `production` as `current` (being built) or in `queued` (waiting), it is handled. Your only job is to wait for it.
- Ordering a building again does not speed it up — it just wastes future money.
- While a building is in progress, spend your turn on your ARMY and your ATTACK instead of touching the build queue.
